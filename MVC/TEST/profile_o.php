...
// Lấy danh sách các item chat
function openChatDetail(idUser_chat, userName, avatarUrl) {
    ...
    // Gửi yêu cầu AJAX để lấy tin nhắn
    fetch("/MVC/Process/message_process.php", {
        ...
        body: JSON.stringify({
            idUser_chat: idUser_chat
        })
    })
    .then(response => response.json())
    .then(data => {
        renderMessages(data); // Gọi hàm renderMessages để hiển thị
    })
    .catch(error => {
...
function renderMessages(data) {
    ...
    if (data.length > 0) {
        data.forEach(message => {
            ...
            // Xác định tin nhắn là của người gửi hay người nhận
...
function sendMessage() {
    ...
    if (message) {
        ...
        const data = {
            sender_id: '6', // ID người gửi
            receiver_id: '8', // ID người nhận (lấy từ data-idUserChat khi click)
            message_content: message, // Nội dung tin nhắn
           // avatar_user_chat: avatarUrl, // Avatar của người gửi
